...
public class FileMongoServiceImpl implements FileService {
    ...
    @Override
    public ServiceResult<String> saveFile(FileDTO fileDTO) {
        byte[] data = fileDTO.getData();
        if (data != null && data.length > 0) {
            InputStream in = new ByteArrayInputStream(data);
            ObjectId objectId = gridFsTemplate.store(in, fileDTO.getFileName(), fileDTO.getContentType(), fileDTO.getAttrs());
            return ServiceResult.success(objectId.toString());
        } else {
    ...
    @Override
    public ServiceResult<FileDTO> getFile(String fileId) {
        return getFile(fileId, null);
    }

    @Override
    public ServiceResult<FileDTO> getFile(String fileId, String httpHeaderRange) {
        ...
        return ServiceResult.success(fileDTO);
    }
