...
public class BigDecimalUtil {
     ...
     * @return
     */
    public static String div2Str(double value1, double value2) {
        BigDecimal b1 = new BigDecimal(Double.toString(value1));
        BigDecimal b2 = new BigDecimal(Double.toString(value2));
        return b1.divide(b2, DEFAULT_SCALE, DEFAULT_ROUND_MODE).toString();
    }

    /**
     * 整形取小数位，常用于金额
     * <p> point=2：2000 >>> 20.00
     * @param val
     * @param point
     * @return
     */
    public static String getCertainPoint(long val, int point) {
        if (point < 0) {
            point = DEFAULT_SCALE;
        }
    ...
    public static double retainDecimal(double original, int scale) {
        ...
        return b.setScale(scale, DEFAULT_ROUND_MODE).doubleValue();
    }


    public static void main(String[] args) {

        System.out.println("四舍五入：" + retainDecimal(3.758, 2));
        ...
        System.out.println("除法返回string" + div2Str(1500, 100)); // 如果要15.000这种效果，把doubleValue()改成toString()

        System.out.println("分转元：" + getCertainPoint(2000, 2));

        String n = "1524156781705602397";
        BigDecimal ll = new BigDecimal(n);
        System.out.println("num size: " + n.length() + " bd: " + ll.longValue()); // -6922976412290386228
    }
